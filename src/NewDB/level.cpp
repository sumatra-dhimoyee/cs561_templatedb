...
Level<K,V> :: Level(SST<K,V> sst, uint8_t _no_runs, size_t _level_size, uint8_t level, std::vector<zone<K>>& fp)
{
...
template<typename K, typename V>
bool add_sst(SST<K,V> sst, bool merge, std::vector<zone<K>>& fp);
{
    // Build_SST<K,V> sst_builder = Build_SST<K,V>(entry_vector, this->run_size,  this->level);
    // SST<K,V> sst = sst_builder.build();
    ...
    if(merge)
    {
        SST<K,V> merged_sst = Build_SST<int, int>::merge_sst(this->sst_vector[0], sst, fp);
        this->clear();
...
template<typename K, typename V>
SST<K,V> Level<K,V>::merge_runs(std::vector<zone<K>>& fp)
{
    ...
    {
    SST<K,V> sst = Build_SST<int,int>::merge_sst(merge_sst, this->sst_vector[i], fp);
    merge_sst = sst;
    i++;
    }
